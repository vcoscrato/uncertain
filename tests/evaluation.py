...
from uncertain.metrics import rmse_score, error_uncertainty_correlation, rpi_score, classification, get_hits


def uncertainty_distributions(model, size, upper_bound):

    users = torch.randint(0, model.num_users, (size,), device=model.device)
    items = torch.randint(0, model.num_items, (size,), device=model.device)
    data = Interactions(torch.vstack((users, items)).T)
    uncertainties = model.predict_interactions(data)[1].cpu().detach().numpy()
...
def evaluate_ratings(model, test):
    ...
    predictions = model.predict_interactions(test, batch_size=1e6)

    if not model.is_uncertain:
        model.evaluation['Ratings']['RMSE'] = rmse_score(predictions, test.ratings)
    else:
        model.evaluation['Ratings']['RMSE'] = rmse_score(predictions[0], test.ratings)

        error = torch.abs(test.ratings - predictions[0])
...
def evaluate_recommendations(model, test, train, relevance_threshold=4, max_k=10):
    ...
    ndcg = []
    rri = []
    arange = torch.arange(1, max_k + 1, device=model.device, dtype=torch.float64)
    ...
    for user_id in range(test.num_users):
        ...
        rec = model.recommend(user_id, train)
    ...
    model.evaluation['Recommendations']['Precision'] = torch.vstack(precision).mean(axis=0)
    model.evaluation['Recommendations']['Recall'] = torch.vstack(recall).mean(axis=0)
    model.evaluation['Recommendations']['NDCG'] = torch.vstack(ndcg).mean(axis=0)
    if len(rri) > 0:
