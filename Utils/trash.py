...
from Utils.metrics import rmse_rpi_score, rpi_score
...
class NNBias(object):
    ...
    def __init__(self,
        ...
        set_seed(self._random_state.randint(-10**8, 10**8),
                 cuda=self._use_cuda)
        self._threshold = torch.nn.Threshold(0, 0)
    ...
    def fit(self, interactions, verbose=False):
        ...
        for epoch_num in range(self._n_iter):
            ...
            self._net.user_biases.weight = torch.nn.Parameter(self._threshold(self._net.user_biases.weight))
            self._net.item_biases.weight = torch.nn.Parameter(self._threshold(self._net.item_biases.weight))
...
class EnsembleRecommender(object):
    ...
    def _get_min_max(self, train):
        ...
        rel = predictions.std(axis=1)
        self._min, self._max = min(rel), max(rel)

    def _reliabilities(self, rel):
        rel = np.maximum(np.minimum(rel, self._max), self._min)
        return 1 - ((rel - self._min) / (self._max - self._min))
    ...
    def predict(self, user_ids, item_ids=None):
        ...
        reliabilities = self._reliabilities(predictions.std(axis=1))
...
class ResampleRecommender(object):
    def __init__(self, base_model, n_models):
        ...
        self.rmse = base_model.rmse
        self.rpi = [0]
        self.precision = base_model.precision
        self.recall = base_model.recall
        self._min = 0
        self._max = np.infty
...
class ModelWrapper(object):
    ...
    def fit(self, train):
        ...
        item_ids = train.item_ids
        user_ids, item_ids = _predict_process_ids(user_ids, item_ids,
                                                  self.R[0]._num_items, self.R[0]._use_cuda)

        rel = np.maximum(self.E.predict(user_ids, item_ids), 0.1)
        self._min, self._max = min(rel), max(rel)
    ...
    def predict(self, user_ids, item_ids=None):
        ...
        reliabilities = self._reliabilities(np.maximum(self.E.predict(user_ids, item_ids), 0.1))
        return estimates, reliabilities