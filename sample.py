...
from Uncertain import SamplingSVD
...
model = SamplingSVD(n_epochs=epochs, n_factors=factors, n_models=ensemble, verbose=False).fit(data)
models = deepcopy(model.models)
eRMSE = np.empty((ensemble))
eRPI = np.empty((ensemble-1))
for i in range(ensemble):
    model.models = models[:i+1]
...
ax.set_xlabel('Number of resamples', Fontsize=20)
...
f.tight_layout()
f.savefig('Results/sample/Ensemble size.pdf')
print('Model RMSE:', eRMSE[-1])
print('Full RPI:', eRPI[-1])
...
f.tight_layout()
f.savefig('Results/sample/Rel vs Error.pdf')
...
f.tight_layout()
f.savefig('Results/sample/Interval width.pdf')
