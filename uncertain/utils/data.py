...
class ImplicitData(LightningDataModule):
    ...
    def __init__(self, data, users_on_test=None, test_ratio=0.2, val_ratio=0.2):
        super().__init__()
        ...
        # Heuristic measures
        self.user_support = self.train_val.groupby('user').size().to_numpy()
        self.item_support = self.train_val.groupby('item').size()
        ...
        self.item_support = self.item_support.append(empty).sort_index().fillna(0)
        
        # Training arrays
        self.train = self.train_val.drop(index=val.index).to_numpy()
        self.test = test.to_numpy()
        self.val = val.to_numpy()
        self.train_val = [[u, self.train[self.train[:, 0] == u, 1], self.val[self.val[:, 0] == u, 1]] for u in self.test_users]
